...
function AppContent() {
  const { user, loading } = useAuth();
  const location = useLocation(); // Used to conditionally hide FAB on certain pages and when viewing requests
  const navigate = useNavigate(); // Used for FAB navigation to /create and for redirection in RequireAuth
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-[#09090b] font-sans text-zinc-900 dark:text-zinc-100 
    transition-colors duration-300">
      ...
      <div className="page-enter">
        <Routes>
          ...
          {/* Fallback - Redirect any unknown routes back to the landing page */}
      ...
      {showFab && (
        <button
          ...
          className="md:hidden fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-full shadow-[0_10px_30px_rgba(16,185,129,0.4)] flex items-center justify-center hover:shadow-[0_15px_35px_rgba(16,185,129,0.5)] active:scale-90 transition-all z-40"
